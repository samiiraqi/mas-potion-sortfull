...
const ClassicBottle = memo(function ClassicBottle({
  colors,
  position,
  onSelect,
  isSelected,
  ...
  const strokeColor = isSelected ? '#FFD700' : isFull ? '#00FF00' : 'rgba(255,255,255,0.8)';
  ...
  return (
    ...
    >
      ...
      <svg width={BOTTLE_WIDTH} height={BOTTLE_HEIGHT} style={{ overflow: 'visible' }}>
        <defs>
          <linearGradient id={`classic-shine-${position.x}-${position.y}`} x1="0%" y1="0%" x2="100%" y2="0%">
          ...
          <clipPath id={`classic-clip-${position.x}-${position.y}`}>
            <path d={`
        ...
        <g clipPath={`url(#classic-clip-${position.x}-${position.y})`}>
        ...
        <ellipse
          ...
          fill={`url(#classic-shine-${position.x}-${position.y})`}
        />
